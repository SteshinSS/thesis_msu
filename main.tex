...
\begin{itemize}
	\item Меченность входного субстрата (например, в каких позициях глюкозы стояли тяжелые изотопы \ce{^{13}C}).
...
Существует несколько программ для \ce{^{13}C}-MFA расчетов (См. приложение А). Дописать после того, как наберу программ.
...
\emph{Трейсер} --- Атом, тяжелый стабильный изотоп которого отслеживается в MFA. Обычно, это \ce{^{13}C}.

\emph{MID} --- Mass Isotope Distribution, вектор $M\!I\!D = [M_0, M_1, \ldots, M_n]$, соответствующим метаболиту $M$, где $M_i$ --- массовая доля метаболита с $i$ атомами трейсера, и $\sum_{i = 0}^{n} = 1$.
...
\emph{Стехиометрическая матрица} --- Матрица, каждая строка которой соответствует химическому уравнению и заполнен коэффициентами метаболитов. При условии стационарности, при умножении на столбец скоростей метаболитов\todo{не так называется} даст ноль.

\clearpage

\section{Допущения}
Математическая модель для \ce{^{13}C}-MFA основывается на нескольких допущениях о биологической системе\cite{Wiechert_1997_1}:\todo{Добавить метки}
\begin{enumerate}
...
Пусть $A$ --- молекула. Любое непустое подмножество атомов углерода молекулы $A$ будем называть \emph{Elementary Metabolic Unit} (далее \emph{EMU}). Например, если $A$ состоит из трех атомов углерода, обозначим через $A_{13}$ EMU состоящее из первого и третьего атома углерода (на атомах углерода одной молекулы существует естественный порядок).
...
Из допущения \todo{какого?}, система находится в стационарном состоянии. Значит скорость реакции не меняется:
\begin{center}
...
\begin{appendices}
	...
	Дадим определение \emph{химической реакции}. Пусть $U, W \subset V$ --- конечные мультимножества метаболитов. Рассмотрим упорядоченную пару $e = (U, W)$, элементы которой назовем \emph{субстратом} и \emph{продуктом} соответственно. Пусть:
	\begin{itemize}
		\item Количество атомов трейсера одинаково в субстрате и продукте: \\ $n = \sum_{u \in U} \mathbb{C}(u) = \sum_{w \in W} \mathbb{C}(w_j)$. 
		\item Задана перестановка $S(e)$ с мощностью, равной количеству атомов трейсера в субстрате и продукте $n$.
	\end{itemize}
	
	\emph{Метболической сетью} будем называть направленный гиперграф $G = (V, E)$, такой что каждое ребро $e \in E$ является химической реакцией.
\end{appendices}
