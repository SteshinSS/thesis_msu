...
\begin{appendices}
	...
	Пусть $V$ -- конечное множество \emph{метаболитов}. Для каждого метаболита известно количество атомов трейсера: $\mathbb{C} \colon V \to \mathbb{N}$. \emph{Химической реакцией} $e$ назовем пару $e\colon (\{V \times \mathbb{R} \}^n, \{V \times \mathbb{R} \}^m)$, элементы которой называют \emph{субстратом} и \emph{продуктом} соответственно и такую, что количество атомов трейсера одинаково в субстрате и продукте: $\sum_{i = 1}^{n} \mathbb{C}(v_i) = \sum_{j = 1}^{m} \mathbb{C}(u_j)$.
\end{appendices}
